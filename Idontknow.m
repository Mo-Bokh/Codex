...
SDm = C(418:668 , (locs(1)- (w-52) - (sn-1) *inc ):(locs(1) + 51 - (sn-1) * inc)); % [ 25 Point from the middle part of each frame ]
...
if(i==1)
    ZZ(418:668 , (locs(1)- (w-52) - (sn-1) *inc):(locs(1)+ 51 - (sn-1) * inc)) = (Zm.*700) ;      
    ZZ = C + ZZ;          
...
Fm(418:668 , (locs(1)- (w-52) - (sn-1) *inc):(locs(1)+ 51 - (sn-1) * inc)) = Xm;
...
% ---- Cut matrix from each frame -----   
SDr = C(418:668 ,(locs(1)- (w-42) - (sn-1) *inc )+ sShift:(locs(1) + 41 - (sn-1) * inc) + sShift) ;
...
%Showcase area of interest
if(i==1)
    ZZr(418:668 , (locs(1)- (w-42) - (sn-1) *inc )+ sShift:(locs(1) + 41 - (sn-1) * inc) + sShift) = (Zr.*700) ;
    ZZr = C + ZZr;
    imgr(:,:,sn) = ZZr(400:699 , 1 : 400);
end
% ---- Apply mask on data ----
Xr = SDr .* Zr;

% ----- Insert masked data in the empty matrix -----
Fr(418:668 , (locs(1)- (w-42) - (sn-1) *inc )+ sShift:(locs(1) + 41 - (sn-1) * inc) + sShift) = Xr;
